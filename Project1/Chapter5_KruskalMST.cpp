...
public:
	...
	void add_edge(Edge<T>&& e)
	{
		if (e.src >= 1 && e.src <= V && e.dst >= 1 && e.dst <= V)
			...
			cerr << "Error : Edges with out of index!" << endl;
	}
...
Graph<T> minimum_spanning_tree(const Graph<T>& G)
{
	...
	//모든 에지를 최소 힙에 추가, 위에 대소관계 operator를 overloading 해서 heap에 weight 순으로 정렬되서 들어감
	for (auto& e : G.edges())
...
int main()
{
	...
	for (auto& i : edge_map)
		for (auto& j : i.second)//i.first는 key , i.second는 value (map을 for auto로 접근할시)
